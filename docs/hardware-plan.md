...
- [ ] **Deep-sleep current is the gating measurement — take it first.** A stock dev board draws 5–20 mA asleep because of its LDO and USB-serial chip; at 10 mA that's 240 mAh/day doing nothing, which exceeds the wake-cycle cost of *every* candidate reporting cadence and pins runtime near 10 days no matter what the firmware does. The custom Logic & Power board should reach 20–50 µA. Until that gap is measured and closed, reporting-cadence tuning is meaningless
  - The overhead isn't the ESP32 itself (which can reach low-µA deep sleep on its own) — it's the dev board's onboard LDO quiescent draw and its USB-serial chip (CP2102/CH340) staying powered even off-USB. The fix is the buck-boost rail in [hardware/logic-power-board-schematic.md](../hardware/logic-power-board-schematic.md), which feeds the socketed ESP32 DevKit's 3V3 pin directly, bypassing the DevKit's own regulator/USB circuitry as the current-draw path. That circuit is still bench-unvalidated (see the BOM notes there) — this phase's measurement is what confirms it actually reaches the 20–50 µA target
