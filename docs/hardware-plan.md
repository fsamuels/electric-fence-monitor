...
- [ ] Derive the per-node calibration constant/curve; record it in `docs/calibration.md` keyed on **`(node ID, chip_id, date)`** — the constant depends on a specific board's ADC as well as the divider chain, so a board swap invalidates it. Keep the raw measurement pairs, not just the derived constants, so the fit can be redone later
