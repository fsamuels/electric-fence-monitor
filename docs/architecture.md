...
- **`docs/software-plan.md`'s own phase checkboxes are partially stale** relative to actual progress — it points at `dashboard-plan.md` for Phase 5 status (which is current), and Firmware Phases 2–3 are now checked off (`temp_c` correctly left open, blocked on hardware), but Firmware Phase 1 checkboxes are still unchecked despite working code existing for all of it. Treat `dashboard-plan.md` as the authoritative phase tracker for dashboard work and `firmware/README.md` for firmware status; `software-plan.md`/`hardware-plan.md` are the higher-level plans, not up-to-date trackers.
